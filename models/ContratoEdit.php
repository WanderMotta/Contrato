...
class ContratoEdit extends Contrato
{
    ...
    // CSS class/style
    public $CurrentPageName = "ContratoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idcontrato") ?? Key(0) ?? Route(2)) !== null) {
                $this->idcontrato->setQueryStringValue($keyValue);
                $this->idcontrato->setOldValue($this->idcontrato->QueryStringValue);
            } elseif (Post("idcontrato") !== null) {
                $this->idcontrato->setFormValue(Post("idcontrato"));
                $this->idcontrato->setOldValue($this->idcontrato->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("idcontrato") ?? Route("idcontrato")) !== null) {
                    ...
                    $this->idcontrato->CurrentValue = null;
                }
            }
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContratoList") {
                            ...
                            $returnUrl = "ContratoList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idcontrato->IsDetailKey) {
        ...
        // Check field name 'valor_antes' first before field var 'x_valor_antes'
        $val = $CurrentForm->hasValue("valor_antes") ? $CurrentForm->getValue("valor_antes") : $CurrentForm->getValue("x_valor_antes");
        if (!$this->valor_antes->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->valor_antes->Visible = false; // Disable update for API request
            } else {
                $this->valor_antes->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->valor_antes->CurrentValue = $this->valor_antes->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        // idcontrato
        $this->idcontrato->RowCssClass = "row";
        ...
        // valor_antes
        $this->valor_antes->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->valor_antes->Visible && $this->valor_antes->Required) {
                if (!$this->valor_antes->IsDetailKey && EmptyValue($this->valor_antes->FormValue)) {
                    $this->valor_antes->addErrorMessage(str_replace("%s", $this->valor_antes->caption(), $this->valor_antes->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("planilha_custo_contrato", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MovInsumoContratoGrid");
        if (in_array("mov_insumo_contrato", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PlanilhaCustoContratoGrid");
            if (in_array("planilha_custo_contrato", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "planilha_custo_contrato"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("MovInsumoContratoGrid");
            if (in_array("mov_insumo_contrato", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "mov_insumo_contrato"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
