 ...
 * Page class
 */
class CargoCopyList extends CargoCopy
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CargoCopyList";
    ...
    public $FormName = "fcargo_copylist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "CargoCopyList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CargoCopyAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "CargoCopyDelete";
        $this->MultiUpdateUrl = "CargoCopyUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->salario1->AdvancedSearch->toJson(), ","); // Field salario1
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcargo_copysrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field salario1
        $this->salario1->AdvancedSearch->SearchValue = @$filter["x_salario1"];
        $this->salario1->AdvancedSearch->SearchOperator = @$filter["z_salario1"];
        $this->salario1->AdvancedSearch->SearchCondition = @$filter["v_salario1"];
        $this->salario1->AdvancedSearch->SearchValue2 = @$filter["y_salario1"];
        $this->salario1->AdvancedSearch->SearchOperator2 = @$filter["w_salario1"];
        $this->salario1->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nr_horas_mes, $ctrl); // nr_horas_mes
            ...
            $this->updateSort($this->nr_horas_ad_noite, $ctrl); // nr_horas_ad_noite
            $this->updateSort($this->intrajornada, $ctrl); // intrajornada
            $this->updateSort($this->tipo_uniforme_idtipo_uniforme, $ctrl); // tipo_uniforme_idtipo_uniforme
            $this->updateSort($this->salario_antes, $ctrl); // salario_antes
            $this->updateSort($this->vt_dia_antes, $ctrl); // vt_dia_antes
            $this->updateSort($this->vr_dia_antes, $ctrl); // vr_dia_antes
            $this->updateSort($this->va_mes_antes, $ctrl); // va_mes_antes
            $this->updateSort($this->benef_social_antes, $ctrl); // benef_social_antes
            $this->updateSort($this->plr_antes, $ctrl); // plr_antes
            $this->updateSort($this->assis_medica_antes, $ctrl); // assis_medica_antes
            $this->updateSort($this->assis_odonto_antes, $ctrl); // assis_odonto_antes
            $this->updateSort($this->funcao_idfuncao, $ctrl); // funcao_idfuncao
            $this->updateSort($this->salario1, $ctrl); // salario1
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"cargo_copy\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cargo_copy\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"cargo_copy\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcargo_copylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcargo_copylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"cargo_copy\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "nr_horas_mes");
            ...
            $this->createColumnOption($option, "nr_horas_ad_noite");
            $this->createColumnOption($option, "intrajornada");
            $this->createColumnOption($option, "tipo_uniforme_idtipo_uniforme");
            $this->createColumnOption($option, "salario_antes");
            $this->createColumnOption($option, "vt_dia_antes");
            $this->createColumnOption($option, "vr_dia_antes");
            $this->createColumnOption($option, "va_mes_antes");
            $this->createColumnOption($option, "benef_social_antes");
            $this->createColumnOption($option, "plr_antes");
            $this->createColumnOption($option, "assis_medica_antes");
            $this->createColumnOption($option, "assis_odonto_antes");
            $this->createColumnOption($option, "funcao_idfuncao");
            $this->createColumnOption($option, "salario1");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcargo_copysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcargo_copysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcargo_copylist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cargo_copy", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_cargo_copy",
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcargo_copylist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcargo_copysrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
