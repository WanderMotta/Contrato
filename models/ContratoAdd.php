...
class ContratoAdd extends Contrato
{
    ...
    // CSS class/style
    public $CurrentPageName = "ContratoAdd";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("planilha_custo_contrato", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MovInsumoContratoGrid");
        if (in_array("mov_insumo_contrato", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PlanilhaCustoContratoGrid");
            if (in_array("planilha_custo_contrato", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->contrato_idcontrato->setSessionValue($this->idcontrato->CurrentValue); // Set master key
                ...
                if (!$addRow) {
                $detailPage->contrato_idcontrato->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("MovInsumoContratoGrid");
            if (in_array("mov_insumo_contrato", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->contrato_idcontrato->setSessionValue($this->idcontrato->CurrentValue); // Set master key
