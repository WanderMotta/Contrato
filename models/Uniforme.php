...
/**
 * Table class for uniforme
 */
class Uniforme extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // iduniforme
        $this->iduniforme = new DbField(
            $this, // Table
            'x_iduniforme', // Variable name
            'iduniforme', // Name
            '`iduniforme`', // Expression
            '`iduniforme`', // Basic search expression
            19, // Type
            3, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`iduniforme`', // Virtual expression
            false, // Is virtual
        ...
        $this->iduniforme->InputTextType = "text";
        $this->iduniforme->Raw = true;
        $this->iduniforme->IsAutoIncrement = true; // Autoincrement field
        $this->iduniforme->IsPrimaryKey = true; // Primary key field
        $this->iduniforme->Nullable = false; // NOT NULL field
        $this->iduniforme->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->iduniforme->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['iduniforme'] = &$this->iduniforme;
        ...
        $this->periodo_troca->Lookup = new Lookup($this->periodo_troca, 'uniforme', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->periodo_troca->OptionCount = 5;
        $this->periodo_troca->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->periodo_troca->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->vr_total = new DbField(
            ...
            'vr_total', // Name
            '(qtde*vr_unitario*periodo_troca)', // Expression
            '(qtde*vr_unitario*periodo_troca)', // Basic search expression
            131, // Type
            32, // Size
            -1, // Date/Time format
            false, // Is upload field
            '(qtde*vr_unitario*periodo_troca)', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "uniforme";
    }
    ...
    {
        return "*, (qtde*vr_unitario*periodo_troca) AS `vr_total`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->iduniforme->setDbValue($conn->lastInsertId());
            $rs['iduniforme'] = $this->iduniforme->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['iduniforme']) && !EmptyValue($this->iduniforme->CurrentValue)) {
                $rs['iduniforme'] = $this->iduniforme->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('iduniforme', $rs)) {
                AddFilter($where, QuotedName('iduniforme', $this->Dbid) . '=' . QuotedValue($rs['iduniforme'], $this->iduniforme->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->iduniforme->DbValue = $row['iduniforme'];
    ...
    {
        $keys = [];
        $val = $current ? $this->iduniforme->CurrentValue : $this->iduniforme->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->iduniforme->CurrentValue = $keys[0];
            } else {
                $this->iduniforme->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('iduniforme', $row) ? $row['iduniforme'] : null;
        } else {
            $val = !EmptyValue($this->iduniforme->OldValue) && !$current ? $this->iduniforme->OldValue : $this->iduniforme->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@iduniforme@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("UniformeList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "UniformeView" => $Language->phrase("View"),
            "UniformeEdit" => $Language->phrase("Edit"),
            "UniformeAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "UniformeView",
            Config("API_ADD_ACTION") => "UniformeAdd",
            Config("API_EDIT_ACTION") => "UniformeEdit",
            Config("API_DELETE_ACTION") => "UniformeDelete",
            Config("API_LIST_ACTION") => "UniformeList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("UniformeView", $parm);
        } else {
            $url = $this->keyUrl("UniformeView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UniformeEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UniformeList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UniformeAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UniformeList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("UniformeDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"iduniforme\":" . VarToJson($this->iduniforme->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->iduniforme->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->iduniforme->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("iduniforme") ?? Route("iduniforme")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->iduniforme->CurrentValue = $key;
            } else {
                $this->iduniforme->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // iduniforme
        $this->iduniforme->HrefValue = "";
        $this->iduniforme->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // iduniforme
        $this->iduniforme->setupEditAttributes();
        $this->iduniforme->EditValue = $this->iduniforme->CurrentValue;
    ...
    public function exportDocument($doc, $result, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->iduniforme);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->iduniforme);
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->iduniforme, '');
                ...
                $doc->exportAggregate($this->vr_total, '');
                $doc->endExportRow();
            }
        }
        if (!$doc->ExportCustom) {
