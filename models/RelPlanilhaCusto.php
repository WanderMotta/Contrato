...
/**
 * Table class for rel_planilha_custo
 */
class RelPlanilhaCusto extends ReportTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->idproposta->GroupingFieldId = 1;
        $this->idproposta->ShowGroupHeaderAsRow = $this->ShowGroupHeaderAsRow;
        $this->idproposta->ShowCompactSummaryFooter = $this->ShowCompactSummaryFooter;
        $this->idproposta->GroupByType = "";
        $this->idproposta->GroupInterval = "0";
        $this->idproposta->GroupSql = "";
        ...
        $this->dt_cadastro = new ReportField(
            ...
            CastDateFieldForLike("`dt_cadastro`", 14, "DB"), // Basic search expression
            133, // Type
            10, // Size
            14, // Date/Time format
            false, // Is upload field
        ...
        $this->dt_cadastro->GroupingFieldId = 2;
        $this->dt_cadastro->ShowGroupHeaderAsRow = $this->ShowGroupHeaderAsRow;
        $this->dt_cadastro->ShowCompactSummaryFooter = $this->ShowCompactSummaryFooter;
        $this->dt_cadastro->GroupByType = "";
        $this->dt_cadastro->GroupInterval = "0";
        $this->dt_cadastro->GroupSql = "";
        $this->dt_cadastro->Nullable = false; // NOT NULL field
        $this->dt_cadastro->Required = true; // Required field
        $this->dt_cadastro->DefaultErrorMessage = str_replace("%s", DateFormat(14), $Language->phrase("IncorrectDate"));
        ...
        $this->cliente->GroupingFieldId = 3;
        ...
        $this->modulo->UseFilter = true; // Table header filter
        $this->modulo->Lookup = new Lookup($this->modulo, 'modulo', true, 'modulo', ["modulo","","",""], '', '', [], [], [], [], [], [], false, '`modulo` ASC', '', "`modulo`");
        ...
        $this->item->UseFilter = true; // Table header filter
        $this->item->Lookup = new Lookup($this->item, 'rel_planilha_custo', true, 'item', ["item","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->idplanilha_custo->GroupingFieldId = 4;
        ...
        $this->idplanilha_custo->setSelectMultiple(false); // Select one
        $this->idplanilha_custo->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->idplanilha_custo->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->idplanilha_custo->Lookup = new Lookup($this->idplanilha_custo, 'view_rel_planilha_custo', true, 'idplanilha_custo', ["idplanilha_custo","cliente","cargo",""], '', '', [], [], [], [], [], [], false, '`idplanilha_custo` ASC', '', "CONCAT(COALESCE(`idplanilha_custo`, ''),'" . ValueSeparator(1, $this->idplanilha_custo) . "',COALESCE(`cliente`,''),'" . ValueSeparator(2, $this->idplanilha_custo) . "',COALESCE(`cargo`,''))");
    ...
    public function getSqlFirstGroupField($alias = false)
    {
        ...
        $firstGroupField = &$this->idproposta;
    ...
    {
        return $this->sqlSelectAggregate ?? $this->getQueryBuilder()->select("SUM(`porcentagem`) AS sum_porcentagem, SUM(`valor`) AS sum_valor");
    }
    ...
    {
        $this->idplanilha_custo->ViewValue = GetDropDownDisplayValue($this->idplanilha_custo->CurrentValue, "", 0);
    }
    ...
    {
        return "RelPlanilhaCusto";
    }

    // API page name
    public function getApiPageName($action)
    {
        return "RelPlanilhaCustoSummary";
    }
