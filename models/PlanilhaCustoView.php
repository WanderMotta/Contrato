 ...
 * Page class
 */
class PlanilhaCustoView extends PlanilhaCusto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PlanilhaCustoView";
    ...
    // CSS class/style
    public $CurrentPageName = "PlanilhaCustoView";
    ...
    public function __construct()
    {
        ...
        // Set up record key
        if (($keyValue = Get("idplanilha_custo") ?? Route("idplanilha_custo")) !== null) {
            $this->RecKey["idplanilha_custo"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("idplanilha_custo") ?? Route("idplanilha_custo")) !== null) {
            $this->idplanilha_custo->setQueryStringValue($keyValue);
            $this->RecKey["idplanilha_custo"] = $this->idplanilha_custo->QueryStringValue;
        } elseif (Post("idplanilha_custo") !== null) {
            $this->idplanilha_custo->setFormValue(Post("idplanilha_custo"));
            $this->RecKey["idplanilha_custo"] = $this->idplanilha_custo->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->idplanilha_custo->setQueryStringValue($keyValue);
            $this->RecKey["idplanilha_custo"] = $this->idplanilha_custo->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "PlanilhaCustoList"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "PlanilhaCustoList"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_movimento_pla_custo");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("movimento_pla_custo", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("MovimentoPlaCustoList?" . Config("TABLE_SHOW_MASTER") . "=planilha_custo&" . GetForeignKeyUrl("fk_idplanilha_custo", $this->idplanilha_custo->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("MovimentoPlaCustoGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'planilha_custo')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=movimento_pla_custo"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "movimento_pla_custo";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'planilha_custo')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=movimento_pla_custo"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "movimento_pla_custo";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'planilha_custo')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=movimento_pla_custo"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("movimento_pla_custo", $detailTblVar)) {
                $detailPageObj = Container("MovimentoPlaCustoGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->planilha_custo_idplanilha_custo->IsDetailKey = true;
                    $detailPageObj->planilha_custo_idplanilha_custo->CurrentValue = $this->idplanilha_custo->CurrentValue;
                    $detailPageObj->planilha_custo_idplanilha_custo->setSessionValue($detailPageObj->planilha_custo_idplanilha_custo->CurrentValue);
                    $detailPageObj->calculo_idcalculo->setSessionValue(""); // Clear session key
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PlanilhaCustoList"), "", $this->TableVar, true);
        $pageId = "view";
