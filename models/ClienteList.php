...
class ClienteList extends Cliente
{
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->local_idlocal, $default, false); // local_idlocal
        $this->buildSearchSql($where, $this->cnpj, $default, false); // cnpj
        $this->buildSearchSql($where, $this->endereco, $default, false); // endereco
        $this->buildSearchSql($where, $this->numero, $default, false); // numero
        $this->buildSearchSql($where, $this->bairro, $default, false); // bairro
        $this->buildSearchSql($where, $this->cep, $default, false); // cep
        $this->buildSearchSql($where, $this->cidade, $default, false); // cidade
        $this->buildSearchSql($where, $this->uf, $default, false); // uf
        $this->buildSearchSql($where, $this->contato, $default, false); // contato
    ...
    public function showFilterList()
    {
        ...
        // Field contato
        $filter = $this->queryBuilderWhere("contato");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->contato, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->contato->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field telefone
        $filter = $this->queryBuilderWhere("telefone");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->telefone, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->telefone->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->local_idlocal->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->cnpj->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->cep->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->contato->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->cliente->Expression . " ASC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_faturamento"
        $item = &$this->ListOptions->add("detail_faturamento");
        $item->CssClass = "text-nowrap";
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"cliente\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
        ...
        // "detail_faturamento"
        $opt = $this->ListOptions["detail_faturamento"];
        if ($Security->allowList(CurrentProjectID() . 'faturamento')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("faturamento", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("FaturamentoList?" . Config("TABLE_SHOW_MASTER") . "=cliente&" . GetForeignKeyUrl("fk_idcliente", $this->idcliente->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            ...
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cliente')) {
                ...
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=faturamento");
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cliente')) {
                ...
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=faturamento");
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'cliente')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=faturamento");
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->idcliente->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_faturamento");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=faturamento");
        $detailPage = Container("FaturamentoGrid");
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fclientelist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
    ...
    protected function loadSearchValues()
    {
        ...
        // local_idlocal
        if ($this->local_idlocal->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->local_idlocal->AdvancedSearch->SearchValue != "" || $this->local_idlocal->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cnpj
        if ($this->cnpj->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cnpj->AdvancedSearch->SearchValue != "" || $this->cnpj->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // endereco
        if ($this->endereco->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->endereco->AdvancedSearch->SearchValue != "" || $this->endereco->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // bairro
        if ($this->bairro->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->bairro->AdvancedSearch->SearchValue != "" || $this->bairro->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cep
        if ($this->cep->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cep->AdvancedSearch->SearchValue != "" || $this->cep->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cidade
        if ($this->cidade->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cidade->AdvancedSearch->SearchValue != "" || $this->cidade->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // uf
        if ($this->uf->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->uf->AdvancedSearch->SearchValue != "" || $this->uf->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // contato
        if ($this->contato->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->contato->AdvancedSearch->SearchValue != "" || $this->contato->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // telefone
        if ($this->telefone->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idcliente->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->contato->setupEditAttributes();
            if (!$this->contato->Raw) {
                $this->contato->AdvancedSearch->SearchValue = HtmlDecode($this->contato->AdvancedSearch->SearchValue);
            }
            $this->contato->EditValue = HtmlEncode($this->contato->AdvancedSearch->SearchValue);
            $this->contato->PlaceHolder = RemoveHtml($this->contato->caption());
            ...
            $this->ativo->PlaceHolder = RemoveHtml($this->ativo->caption());
                ...
                $this->cliente->Count = 0; // Initialize count
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['cliente']); // cliente
        $hash .= GetFieldHash($row['contato']); // contato
        $hash .= GetFieldHash($row['email']); // email
        $hash .= GetFieldHash($row['telefone']); // telefone
