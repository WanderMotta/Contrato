 ...
 * Page class
 */
class ItensModuloPreview extends ItensModulo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ItensModuloPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ItensModuloPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "modulo") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=modulo&" . GetForeignKeyUrl("fk_idmodulo", $this->modulo_idmodulo->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "modulo") {
            $this->modulo_idmodulo->setQueryStringValue($keys[0] ?? "");
        }
    }
