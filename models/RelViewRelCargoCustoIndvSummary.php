 ...
 * Page class
 */
class RelViewRelCargoCustoIndvSummary extends RelViewRelCargoCustoIndv
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RelViewRelCargoCustoIndvSummary";
    ...
    public $CurrentPageName = "RelViewRelCargoCustoIndv";
    ...
    public function __construct()
    {
        ...
        // Table object (rel_view_rel_cargo_custo_indv)
        if (!isset($GLOBALS["rel_view_rel_cargo_custo_indv"]) || $GLOBALS["rel_view_rel_cargo_custo_indv"]::class == PROJECT_NAMESPACE . "rel_view_rel_cargo_custo_indv") {
            $GLOBALS["rel_view_rel_cargo_custo_indv"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rel_view_rel_cargo_custo_indv');
        }
    ...
    public function loadRowValues($record)
    {
        ...
        $data["vr_contrato"] = $record['vr_contrato'];
        ...
        $this->idmodulo->setDbValue($record['idmodulo']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) {
            ...
            $rs = $sql->executeQuery();
            $records = $rs?->fetchAll() ?? [];
            $this->total->getSum($records, false);
            ...
            if ($rsagg) {
                $this->quantidade->Count = $this->TotalCount;
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->total->getSum($this->DetailRecords, false);
            }
        }
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            if ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->vr_contrato->groupValue(); // Set up group attribute 2
            }
            if ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 3) {
                $this->RowAttrs["data-group-3"] = $this->cargo->groupValue(); // Set up group attribute 3
            }
            if ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 4) {
                $this->RowAttrs["data-group-4"] = $this->modulo->groupValue(); // Set up group attribute 4
            }
            ...
            // vr_contrato
            $this->vr_contrato->GroupViewValue = $this->vr_contrato->groupValue();
            $this->vr_contrato->GroupViewValue = FormatCurrency($this->vr_contrato->GroupViewValue, $this->vr_contrato->formatPattern());
            $this->vr_contrato->CssClass = "fw-bold";
            $this->vr_contrato->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->vr_contrato->CellCssStyle .= "text-align: right;";
            $this->vr_contrato->GroupViewValue = DisplayGroupValue($this->vr_contrato, $this->vr_contrato->GroupViewValue);
            ...
            if ($dispVal != "") {
                $this->cargo->GroupViewValue = $dispVal;
            }
            $this->cargo->CssClass = "fw-bold";
            $this->cargo->CellCssClass = ($this->RowGroupLevel == 3 ? "ew-rpt-grp-summary-3" : "ew-rpt-grp-field-3");
            ...
            $this->modulo->CellCssClass = ($this->RowGroupLevel == 4 ? "ew-rpt-grp-summary-4" : "ew-rpt-grp-field-4");
            ...
            $this->total->SumViewValue = FormatNumber($this->total->SumViewValue, $this->total->formatPattern());
            $this->total->CssClass = "fw-bold";
            $this->total->CellCssStyle .= "text-align: right;";
            $this->total->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            $this->vr_contrato->GroupViewValue = FormatCurrency($this->vr_contrato->GroupViewValue, $this->vr_contrato->formatPattern());
            $this->vr_contrato->CssClass = "fw-bold";
            $this->vr_contrato->CellCssClass = "ew-rpt-grp-field-2";
            $this->vr_contrato->CellCssStyle .= "text-align: right;";
            $this->vr_contrato->GroupViewValue = DisplayGroupValue($this->vr_contrato, $this->vr_contrato->GroupViewValue);
            if (!$this->vr_contrato->LevelBreak) {
                $this->vr_contrato->GroupViewValue = "";
            } else {
                $this->vr_contrato->LevelBreak = false;
            }
            ...
            $this->cargo->CssClass = "fw-bold";
            $this->cargo->CellCssClass = "ew-rpt-grp-field-3";
            $this->cargo->GroupViewValue = DisplayGroupValue($this->cargo, $this->cargo->GroupViewValue);
            ...
            $this->modulo->GroupViewValue = $this->modulo->groupValue();
            $this->modulo->CssClass = "fw-bold";
            $this->modulo->CellCssClass = "ew-rpt-grp-field-4";
            $this->modulo->GroupViewValue = DisplayGroupValue($this->modulo, $this->modulo->GroupViewValue);
            ...
            $this->total->CssClass = "fw-bold";
            $this->total->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            $this->total->CellCssStyle .= "text-align: right;";
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            // vr_contrato
            $currentValue = $this->vr_contrato->GroupViewValue;
            $viewValue = &$this->vr_contrato->GroupViewValue;
            $viewAttrs = &$this->vr_contrato->ViewAttrs;
            $cellAttrs = &$this->vr_contrato->CellAttrs;
            $hrefValue = &$this->vr_contrato->HrefValue;
            $linkAttrs = &$this->vr_contrato->LinkAttrs;
            $this->cellRendered($this->vr_contrato, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // vr_contrato
            $currentValue = $this->vr_contrato->groupValue();
            $viewValue = &$this->vr_contrato->GroupViewValue;
            $viewAttrs = &$this->vr_contrato->ViewAttrs;
            $cellAttrs = &$this->vr_contrato->CellAttrs;
            $hrefValue = &$this->vr_contrato->HrefValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frel_view_rel_cargo_custo_indvsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->cliente->Visible || $this->cargo->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frel_view_rel_cargo_custo_indvsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frel_view_rel_cargo_custo_indvsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
