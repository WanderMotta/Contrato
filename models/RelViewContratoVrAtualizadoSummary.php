 ...
 * Page class
 */
class RelViewContratoVrAtualizadoSummary extends RelViewContratoVrAtualizado
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RelViewContratoVrAtualizadoSummary";
    ...
    public $CurrentPageName = "RelViewContratoVrAtualizado";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'rel_view_contrato_vr_atualizado';
        $this->TableName = 'rel_view_contrato_vr_atualizado';
        ...
        // Table object (rel_view_contrato_vr_atualizado)
        if (!isset($GLOBALS["rel_view_contrato_vr_atualizado"]) || $GLOBALS["rel_view_contrato_vr_atualizado"]::class == PROJECT_NAMESPACE . "rel_view_contrato_vr_atualizado") {
            $GLOBALS["rel_view_contrato_vr_atualizado"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rel_view_contrato_vr_atualizado');
        }
    ...
    public function loadRowValues($record)
    {
        ...
        $data["vr_cobrado_anterior"] = $record['vr_cobrado_anterior'];
        $data["valor_reajustado"] = $record['valor_reajustado'];
        $data["diferenca"] = $record['diferenca'];
        $data["margem"] = $record['margem'];
        $this->Rows[] = $data;
        $this->idcontrato->setDbValue($record['idcontrato']);
        $this->cliente->setDbValue($record['cliente']);
        $this->vr_cobrado_anterior->setDbValue($record['vr_cobrado_anterior']);
        $this->valor_reajustado->setDbValue($record['valor_reajustado']);
        $this->diferenca->setDbValue($record['diferenca']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) { // Get Page total
            ...
            $this->vr_cobrado_anterior->getSum($records, false);
            $this->valor_reajustado->getSum($records, false);
            ...
            if ($rsagg) {
                $this->idcontrato->Count = $this->TotalCount;
                $this->cliente->Count = $this->TotalCount;
                $this->vr_cobrado_anterior->Count = $this->TotalCount;
                $this->vr_cobrado_anterior->SumValue = $rsagg["sum_vr_cobrado_anterior"];
                $this->valor_reajustado->Count = $this->TotalCount;
                $this->valor_reajustado->SumValue = $rsagg["sum_valor_reajustado"];
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->vr_cobrado_anterior->getSum($this->DetailRecords, false);
                $this->valor_reajustado->getSum($this->DetailRecords, false);
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            // margem
            if ($this->margem->UseFilter && !EmptyValue($this->margem->AdvancedSearch->SearchValue)) {
                if (is_array($this->margem->AdvancedSearch->SearchValue)) {
                    $this->margem->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->margem->AdvancedSearch->SearchValue);
                }
                $this->margem->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->margem->AdvancedSearch->SearchValue);
            }
            ...
            // vr_cobrado_anterior
            $this->vr_cobrado_anterior->SumViewValue = $this->vr_cobrado_anterior->SumValue;
            $this->vr_cobrado_anterior->SumViewValue = FormatCurrency($this->vr_cobrado_anterior->SumViewValue, $this->vr_cobrado_anterior->formatPattern());
            $this->vr_cobrado_anterior->CssClass = "fw-bold";
            $this->vr_cobrado_anterior->CellCssStyle .= "text-align: right;";
            $this->vr_cobrado_anterior->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;

            // valor_reajustado
            $this->valor_reajustado->SumViewValue = $this->valor_reajustado->SumValue;
            $this->valor_reajustado->SumViewValue = FormatCurrency($this->valor_reajustado->SumViewValue, $this->valor_reajustado->formatPattern());
            $this->valor_reajustado->CssClass = "fw-bold";
            $this->valor_reajustado->CellCssStyle .= "text-align: right;";
            $this->valor_reajustado->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            $this->idcontrato->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            $this->vr_cobrado_anterior->ViewValue = FormatCurrency($this->vr_cobrado_anterior->ViewValue, $this->vr_cobrado_anterior->formatPattern());
            $this->vr_cobrado_anterior->CssClass = "fw-bold";
            $this->vr_cobrado_anterior->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            $this->vr_cobrado_anterior->CellCssStyle .= "text-align: right;";

            // valor_reajustado
            $this->valor_reajustado->ViewValue = $this->valor_reajustado->CurrentValue;
            $this->valor_reajustado->ViewValue = FormatCurrency($this->valor_reajustado->ViewValue, $this->valor_reajustado->formatPattern());
            $this->valor_reajustado->CssClass = "fw-bold";
            $this->valor_reajustado->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            $this->valor_reajustado->CellCssStyle .= "text-align: right;";
        ...
        if ($this->RowType == RowType::TOTAL) {
            // vr_cobrado_anterior
            $currentValue = $this->vr_cobrado_anterior->SumValue;
            $viewValue = &$this->vr_cobrado_anterior->SumViewValue;
            $viewAttrs = &$this->vr_cobrado_anterior->ViewAttrs;
            $cellAttrs = &$this->vr_cobrado_anterior->CellAttrs;
            $hrefValue = &$this->vr_cobrado_anterior->HrefValue;
            $linkAttrs = &$this->vr_cobrado_anterior->LinkAttrs;
            $this->cellRendered($this->vr_cobrado_anterior, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // valor_reajustado
            $currentValue = $this->valor_reajustado->SumValue;
            $viewValue = &$this->valor_reajustado->SumViewValue;
            $viewAttrs = &$this->valor_reajustado->ViewAttrs;
            $cellAttrs = &$this->valor_reajustado->CellAttrs;
            $hrefValue = &$this->valor_reajustado->HrefValue;
            $linkAttrs = &$this->valor_reajustado->LinkAttrs;
            $this->cellRendered($this->valor_reajustado, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // idcontrato
            $currentValue = $this->idcontrato->CurrentValue;
            $viewValue = &$this->idcontrato->ViewValue;
            $viewAttrs = &$this->idcontrato->ViewAttrs;
            $cellAttrs = &$this->idcontrato->CellAttrs;
            $hrefValue = &$this->idcontrato->HrefValue;
            $linkAttrs = &$this->idcontrato->LinkAttrs;
            $this->cellRendered($this->idcontrato, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // vr_cobrado_anterior
            $currentValue = $this->vr_cobrado_anterior->CurrentValue;
            $viewValue = &$this->vr_cobrado_anterior->ViewValue;
            $viewAttrs = &$this->vr_cobrado_anterior->ViewAttrs;
            $cellAttrs = &$this->vr_cobrado_anterior->CellAttrs;
            $hrefValue = &$this->vr_cobrado_anterior->HrefValue;
            $linkAttrs = &$this->vr_cobrado_anterior->LinkAttrs;
            ...
            // valor_reajustado
            $currentValue = $this->valor_reajustado->CurrentValue;
            $viewValue = &$this->valor_reajustado->ViewValue;
            $viewAttrs = &$this->valor_reajustado->ViewAttrs;
            $cellAttrs = &$this->valor_reajustado->CellAttrs;
            $hrefValue = &$this->valor_reajustado->HrefValue;
            $linkAttrs = &$this->valor_reajustado->LinkAttrs;
    ...
    protected function setupFieldCount()
    {
        ...
        if ($this->idcontrato->Visible) {
            $this->DetailColumnCount += 1;
        }
        if ($this->cliente->Visible) {
            $this->DetailColumnCount += 1;
        }
        if ($this->vr_cobrado_anterior->Visible) {
            $this->DetailColumnCount += 1;
        }
        if ($this->valor_reajustado->Visible) {
            $this->DetailColumnCount += 1;
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frel_view_contrato_vr_atualizadosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->diferenca->Visible || $this->margem->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frel_view_contrato_vr_atualizadosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frel_view_contrato_vr_atualizadosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd") == "reset") {
            ...
            // Field margem
            $this->getDropDownValue($this->margem);
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->diferenca->AdvancedSearch->issetSession()) { // Field diferenca
                $this->diferenca->AdvancedSearch->load();
                $restoreDefault = false;
            }
            if ($this->margem->AdvancedSearch->issetSession()) { // Field margem
        ...
        $this->buildExtendedFilter($this->diferenca, $filter, false, true); // Field diferenca
        $this->diferenca->AdvancedSearch->save();
        $this->buildDropDownFilter($this->margem, $filter, false, true); // Field margem
    ...
    protected function validateForm()
    {
        ...
        if (!Config("SERVER_VALIDATE")) {
            return true;
                ...
                if (!CheckNumber($this->margem->FormValue)) {
                    $this->margem->addErrorMessage($this->margem->getErrorMessage(false));
                }
    ...
    protected function loadDefaultFilters()
    {
        ...
        // Field margem
        $this->margem->AdvancedSearch->loadDefault();
    }
