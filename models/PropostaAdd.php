 ...
 * Page class
 */
class PropostaAdd extends Proposta
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PropostaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "PropostaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "PropostaView") {
    ...
    {
        $this->mes_ano_conv_coletiva->DefaultValue = $this->mes_ano_conv_coletiva->getDefault(); // PHP
        $this->mes_ano_conv_coletiva->OldValue = $this->mes_ano_conv_coletiva->DefaultValue;
        $this->sindicato->DefaultValue = $this->sindicato->getDefault(); // PHP
        $this->sindicato->OldValue = $this->sindicato->DefaultValue;
        $this->cidade->DefaultValue = $this->cidade->getDefault(); // PHP
        $this->cidade->OldValue = $this->cidade->DefaultValue;
        $this->nr_meses->DefaultValue = $this->nr_meses->getDefault(); // PHP
        $this->nr_meses->OldValue = $this->nr_meses->DefaultValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("planilha_custo", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("MovInsumoClienteGrid");
        if (in_array("mov_insumo_cliente", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("PlanilhaCustoGrid");
            if (in_array("planilha_custo", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->proposta_idproposta->setSessionValue($this->idproposta->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "planilha_custo"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->proposta_idproposta->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("MovInsumoClienteGrid");
            if (in_array("mov_insumo_cliente", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->proposta_idproposta->setSessionValue($this->idproposta->CurrentValue); // Set master key
    ...
    protected function getAddRow()
    {
        ...
        // validade
        $this->validade->setDbValueDef($rsnew, UnFormatDateTime($this->validade->CurrentValue, $this->validade->formatPattern()), false);

        // mes_ano_conv_coletiva
        $this->mes_ano_conv_coletiva->setDbValueDef($rsnew, $this->mes_ano_conv_coletiva->CurrentValue, strval($this->mes_ano_conv_coletiva->CurrentValue) == "");

        // sindicato
        $this->sindicato->setDbValueDef($rsnew, $this->sindicato->CurrentValue, strval($this->sindicato->CurrentValue) == "");

        // cidade
        $this->cidade->setDbValueDef($rsnew, $this->cidade->CurrentValue, strval($this->cidade->CurrentValue) == "");

        // nr_meses
        $this->nr_meses->setDbValueDef($rsnew, $this->nr_meses->CurrentValue, strval($this->nr_meses->CurrentValue) == "");
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['validade'])) { // validade
            $this->validade->setFormValue($row['validade']);
        }
        if (isset($row['mes_ano_conv_coletiva'])) { // mes_ano_conv_coletiva
            $this->mes_ano_conv_coletiva->setFormValue($row['mes_ano_conv_coletiva']);
        }
        if (isset($row['sindicato'])) { // sindicato
            $this->sindicato->setFormValue($row['sindicato']);
        }
        if (isset($row['cidade'])) { // cidade
            $this->cidade->setFormValue($row['cidade']);
        }
        if (isset($row['nr_meses'])) { // nr_meses
            $this->nr_meses->setFormValue($row['nr_meses']);
        }
