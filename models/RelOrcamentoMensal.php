...
/**
 * Table class for rel_orcamento_mensal
 */
class RelOrcamentoMensal extends ReportTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->ReportSourceTable = 'view_mov_pla_custo_modulos'; // Report source table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->modulo->Lookup = new Lookup($this->modulo, 'rel_orcamento_mensal', true, 'modulo', ["modulo","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->modulo->SourceTableVar = 'view_mov_pla_custo_modulos';
        ...
        $this->valor->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->valor->SourceTableVar = 'view_mov_pla_custo_modulos';
        ...
        $this->ativo->Raw = true;
        $this->ativo->Lookup = new Lookup($this->ativo, 'rel_orcamento_mensal', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->ativo->OptionCount = 2;
        $this->ativo->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->ativo->SourceTableVar = 'view_mov_pla_custo_modulos';
        $this->Fields['ativo'] = &$this->ativo;

        // Orçamento Mensal
        $this->OrcamentoMensal = new DbChart($this, 'OrcamentoMensal', 'Orçamento Mensal', 'modulo', 'valor', 1001, '', 0, 'SUM', 600, 500);
        $this->OrcamentoMensal->Position = 3;
        $this->OrcamentoMensal->PageBreakType = "before";
        $this->OrcamentoMensal->YAxisFormat = ["Currency"];
        $this->OrcamentoMensal->YFieldFormat = ["Currency"];
        $this->OrcamentoMensal->SortType = 0;
        $this->OrcamentoMensal->SortSequence = "";
        $this->OrcamentoMensal->SqlSelect = $this->getQueryBuilder()->select("`modulo`", "''", "SUM(`valor`)");
        $this->OrcamentoMensal->SqlGroupBy = "`modulo`";
        $this->OrcamentoMensal->SqlOrderBy = "";
        $this->OrcamentoMensal->SeriesDateType = "";
        $this->OrcamentoMensal->ID = "rel_orcamento_mensal_OrcamentoMensal"; // Chart ID
        $this->OrcamentoMensal->setParameters([
            ["type", "1001"],
            ["seriestype", "0"]
        ]); // Chart type / Chart series type
        $this->OrcamentoMensal->setParameters([
            ["caption", $this->OrcamentoMensal->caption()],
            ["xaxisname", $this->OrcamentoMensal->xAxisName()]
        ]); // Chart caption / X axis name
        $this->OrcamentoMensal->setParameter("yaxisname", $this->OrcamentoMensal->yAxisName()); // Y axis name
        $this->OrcamentoMensal->setParameters([
            ["shownames", "1"],
            ["showvalues", "1"],
            ["showhovercap", "1"]
        ]); // Show names / Show values / Show hover
        $this->OrcamentoMensal->setParameter("alpha", DbChart::getDefaultAlpha()); // Chart alpha (datasets background color)
        $this->OrcamentoMensal->setParameters([["options.plugins.legend.display",false],["options.plugins.legend.fullWidth",false],["options.plugins.legend.reverse",false],["options.plugins.legend.rtl",false],["options.plugins.legend.labels.usePointStyle",false],["options.plugins.title.display",false],["options.plugins.tooltip.enabled",true],["options.plugins.tooltip.intersect",false],["options.plugins.tooltip.displayColors",false],["options.plugins.tooltip.rtl",false],["options.plugins.filler.propagate",false],["options.animation.animateRotate",false],["options.animation.animateScale",false],["options.scales.r.angleLines.display",false],["options.plugins.stacked100.enable",false],["dataset.showLine",false],["dataset.spanGaps",false],["dataset.steppedLine",false],["dataset.circular",false],["scale.offset",false],["scale.gridLines.offsetGridLines",false],["options.plugins.datalabels.clamp",false],["options.plugins.datalabels.clip",false],["options.plugins.datalabels.display",false],["annotation1.show",false],["annotation1.secondaryYAxis",false],["annotation2.show",false],["annotation2.secondaryYAxis",false],["annotation3.show",false],["annotation3.secondaryYAxis",false],["annotation4.show",false],["annotation4.secondaryYAxis",false]]);
        $this->Charts[$this->OrcamentoMensal->ID] = &$this->OrcamentoMensal;
    ...
    {
        return $this->sqlSelectAggregate ?? $this->getQueryBuilder()->select("SUM(`valor`) AS sum_valor");
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_mov_pla_custo_modulos";
    }
    ...
    {
        return "RelOrcamentoMensal";
    }

    // API page name
    public function getApiPageName($action)
    {
        return "RelOrcamentoMensalSummary";
    }
