 ...
 * Page class
 */
class RelCrossEfetivoPrevistoCrosstab extends RelCrossEfetivoPrevisto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RelCrossEfetivoPrevistoCrosstab";
    ...
    public $CurrentPageName = "RelCrossEfetivoPrevisto";
    ...
    public function __construct()
    {
        ...
        // Table object (rel_cross_efetivo_previsto)
        if (!isset($GLOBALS["rel_cross_efetivo_previsto"]) || $GLOBALS["rel_cross_efetivo_previsto"]::class == PROJECT_NAMESPACE . "rel_cross_efetivo_previsto") {
            $GLOBALS["rel_cross_efetivo_previsto"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rel_cross_efetivo_previsto');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            // periodo
            if ($this->periodo->UseFilter && !EmptyValue($this->periodo->AdvancedSearch->SearchValue)) {
                if (is_array($this->periodo->AdvancedSearch->SearchValue)) {
                    $this->periodo->AdvancedSearch->SearchValue = implode(Config("FILTER_OPTION_SEPARATOR"), $this->periodo->AdvancedSearch->SearchValue);
                }
                $this->periodo->EditValue = explode(Config("FILTER_OPTION_SEPARATOR"), $this->periodo->AdvancedSearch->SearchValue);
            }
            ...
            // escala
            $this->escala->GroupViewValue = $this->escala->groupValue();
            $this->escala->CssClass = "fw-bold";
            $this->escala->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");

            // periodo
            $this->periodo->GroupViewValue = $this->periodo->groupValue();
            $this->periodo->CssClass = "fw-bold";
            $this->periodo->CellCssClass = ($this->RowGroupLevel == 3 ? "ew-rpt-grp-summary-3" : "ew-rpt-grp-field-3");
            $this->periodo->CellCssStyle .= "text-align: left;";

            // jornada
            $this->jornada->GroupViewValue = $this->jornada->groupValue();
            $this->jornada->GroupViewValue = FormatNumber($this->jornada->GroupViewValue, $this->jornada->formatPattern());
            $this->jornada->CssClass = "fw-bold";
            $this->jornada->CellCssClass = ($this->RowGroupLevel == 4 ? "ew-rpt-grp-summary-4" : "ew-rpt-grp-field-4");
            ...
            for ($i = 0; $i < $scvcnt; $i++) {
                $smry->SummaryViewValues[$i] = FormatNumber($smry->SummaryCurrentValues[$i], $this->quantidade->formatPattern());
                $smry->SummaryViewAttrs[$i]["class"] = "fw-bold";
                $this->SummaryCellAttrs[$i]["style"] = "text-align: center;";
            ...
            $this->escala->GroupViewValue = $this->escala->groupValue();
            $this->escala->CssClass = "fw-bold";
            $this->escala->CellCssClass = "ew-rpt-grp-field-2";
            if (!$this->escala->LevelBreak) {
                $this->escala->GroupViewValue = "";
            } else {
                $this->escala->LevelBreak = false;
            }

            // periodo
            $this->periodo->GroupViewValue = $this->periodo->groupValue();
            $this->periodo->CssClass = "fw-bold";
            $this->periodo->CellCssClass = "ew-rpt-grp-field-3";
            $this->periodo->CellCssStyle .= "text-align: left;";
            if (!$this->periodo->LevelBreak) {
                $this->periodo->GroupViewValue = "";
            } else {
                $this->periodo->LevelBreak = false;
            }
            ...
            $this->jornada->GroupViewValue = FormatNumber($this->jornada->GroupViewValue, $this->jornada->formatPattern());
            $this->jornada->CssClass = "fw-bold";
            $this->jornada->CellCssClass = "ew-rpt-grp-field-4";
            $this->jornada->CellCssStyle .= "text-align: center;";
            if (!$this->jornada->LevelBreak) {
                $this->jornada->GroupViewValue = "";
            } else {
                $this->jornada->LevelBreak = false;
            }
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            $currentValue = $this->escala->groupValue();
            $viewValue = &$this->escala->GroupViewValue;
            ...
            $currentValue = $this->periodo->groupValue();
            $viewValue = &$this->periodo->GroupViewValue;
            ...
            $currentValue = $this->jornada->groupValue();
            $viewValue = &$this->jornada->GroupViewValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frel_cross_efetivo_previstosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->cargo->Visible || $this->cliente->Visible || $this->periodo->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frel_cross_efetivo_previstosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frel_cross_efetivo_previstosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd") == "reset") {
            ...
            // Field periodo
            $this->getDropDownValue($this->periodo);
        ...
        if ($restoreSession) {
            ...
            if ($this->periodo->AdvancedSearch->issetSession()) { // Field periodo
        ...
        $this->buildDropDownFilter($this->periodo, $filter, false, true); // Field periodo
    ...
    protected function loadDefaultFilters()
    {
        ...
        // Field periodo
        $this->periodo->AdvancedSearch->loadDefault();
    }
