 ...
 * Page class
 */
class MovInsumoClienteEdit extends MovInsumoCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MovInsumoClienteEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "MovInsumoClienteEdit";
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "MovInsumoClienteView"); // If View page, no primary button
                    } else { // List page
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idmov_insumo_cliente") ?? Key(0) ?? Route(2)) !== null) {
                $this->idmov_insumo_cliente->setQueryStringValue($keyValue);
                $this->idmov_insumo_cliente->setOldValue($this->idmov_insumo_cliente->QueryStringValue);
            } elseif (Post("idmov_insumo_cliente") !== null) {
                $this->idmov_insumo_cliente->setFormValue(Post("idmov_insumo_cliente"));
                $this->idmov_insumo_cliente->setOldValue($this->idmov_insumo_cliente->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idmov_insumo_cliente") ?? Route("idmov_insumo_cliente")) !== null) {
                    $this->idmov_insumo_cliente->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->idmov_insumo_cliente->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("MovInsumoClienteList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "MovInsumoClienteList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "MovInsumoClienteList") {
                            ...
                            $returnUrl = "MovInsumoClienteList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idmov_insumo_cliente->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        $this->idmov_insumo_cliente->RowCssClass = "row";

        // dt_cadastro
        $this->dt_cadastro->RowCssClass = "row";

        // tipo_insumo_idtipo_insumo
        $this->tipo_insumo_idtipo_insumo->RowCssClass = "row";

        // insumo_idinsumo
        $this->insumo_idinsumo->RowCssClass = "row";
        ...
        // vr_unit
        $this->vr_unit->RowCssClass = "row";

        // vr_total
        $this->vr_total->RowCssClass = "row";

        // proposta_idproposta
        $this->proposta_idproposta->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idmov_insumo_cliente->Visible && $this->idmov_insumo_cliente->Required) {
                if (!$this->idmov_insumo_cliente->IsDetailKey && EmptyValue($this->idmov_insumo_cliente->FormValue)) {
                    $this->idmov_insumo_cliente->addErrorMessage(str_replace("%s", $this->idmov_insumo_cliente->caption(), $this->idmov_insumo_cliente->RequiredErrorMessage));
                }
            }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "proposta") {
                ...
                if (($parm = Get("fk_idproposta", Get("proposta_idproposta"))) !== null) {
                    $masterTbl->idproposta->setQueryStringValue($parm);
                    $this->proposta_idproposta->QueryStringValue = $masterTbl->idproposta->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->proposta_idproposta->setSessionValue($this->proposta_idproposta->QueryStringValue);
                    $foreignKeys["proposta_idproposta"] = $this->proposta_idproposta->QueryStringValue;
                    if (!is_numeric($masterTbl->idproposta->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "proposta") {
                $validMaster = true;
                $masterTbl = Container("proposta");
                if (($parm = Post("fk_idproposta", Post("proposta_idproposta"))) !== null) {
                    $masterTbl->idproposta->setFormValue($parm);
                    $this->proposta_idproposta->FormValue = $masterTbl->idproposta->FormValue;
                    $this->proposta_idproposta->setSessionValue($this->proposta_idproposta->FormValue);
                    $foreignKeys["proposta_idproposta"] = $this->proposta_idproposta->FormValue;
                    if (!is_numeric($masterTbl->idproposta->FormValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "proposta") {
                if (!array_key_exists("proposta_idproposta", $foreignKeys)) { // Not current foreign key
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MovInsumoClienteList"), "", $this->TableVar, true);
        $pageId = "edit";
