 ...
 * Page class
 */
class PropostaList extends Proposta
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PropostaList";
    ...
    public $FormName = "fpropostalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "PropostaList";
    ...
    public function setVisibility()
    {
        ...
        $this->validade->setVisibility();
        $this->mes_ano_conv_coletiva->setVisibility();
        $this->sindicato->setVisibility();
        $this->cidade->setVisibility();
        $this->nr_meses->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (proposta)
        if (!isset($GLOBALS["proposta"]) || $GLOBALS["proposta"]::class == PROJECT_NAMESPACE . "proposta") {
            $GLOBALS["proposta"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "PropostaAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "PropostaDelete";
        $this->MultiUpdateUrl = "PropostaUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'proposta');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "PropostaView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->validade->AdvancedSearch->toJson(), ","); // Field validade
        $filterList = Concat($filterList, $this->mes_ano_conv_coletiva->AdvancedSearch->toJson(), ","); // Field mes_ano_conv_coletiva
        $filterList = Concat($filterList, $this->sindicato->AdvancedSearch->toJson(), ","); // Field sindicato
        $filterList = Concat($filterList, $this->cidade->AdvancedSearch->toJson(), ","); // Field cidade
        $filterList = Concat($filterList, $this->nr_meses->AdvancedSearch->toJson(), ","); // Field nr_meses
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpropostasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field validade
        $this->validade->AdvancedSearch->SearchValue = @$filter["x_validade"];
        $this->validade->AdvancedSearch->SearchOperator = @$filter["z_validade"];
        $this->validade->AdvancedSearch->SearchCondition = @$filter["v_validade"];
        $this->validade->AdvancedSearch->SearchValue2 = @$filter["y_validade"];
        $this->validade->AdvancedSearch->SearchOperator2 = @$filter["w_validade"];
        $this->validade->AdvancedSearch->save();

        // Field mes_ano_conv_coletiva
        $this->mes_ano_conv_coletiva->AdvancedSearch->SearchValue = @$filter["x_mes_ano_conv_coletiva"];
        $this->mes_ano_conv_coletiva->AdvancedSearch->SearchOperator = @$filter["z_mes_ano_conv_coletiva"];
        $this->mes_ano_conv_coletiva->AdvancedSearch->SearchCondition = @$filter["v_mes_ano_conv_coletiva"];
        $this->mes_ano_conv_coletiva->AdvancedSearch->SearchValue2 = @$filter["y_mes_ano_conv_coletiva"];
        $this->mes_ano_conv_coletiva->AdvancedSearch->SearchOperator2 = @$filter["w_mes_ano_conv_coletiva"];
        $this->mes_ano_conv_coletiva->AdvancedSearch->save();

        // Field sindicato
        $this->sindicato->AdvancedSearch->SearchValue = @$filter["x_sindicato"];
        $this->sindicato->AdvancedSearch->SearchOperator = @$filter["z_sindicato"];
        $this->sindicato->AdvancedSearch->SearchCondition = @$filter["v_sindicato"];
        $this->sindicato->AdvancedSearch->SearchValue2 = @$filter["y_sindicato"];
        $this->sindicato->AdvancedSearch->SearchOperator2 = @$filter["w_sindicato"];
        $this->sindicato->AdvancedSearch->save();
        ...
        // Field nr_meses
        $this->nr_meses->AdvancedSearch->SearchValue = @$filter["x_nr_meses"];
        $this->nr_meses->AdvancedSearch->SearchOperator = @$filter["z_nr_meses"];
        $this->nr_meses->AdvancedSearch->SearchCondition = @$filter["v_nr_meses"];
        $this->nr_meses->AdvancedSearch->SearchValue2 = @$filter["y_nr_meses"];
        $this->nr_meses->AdvancedSearch->SearchOperator2 = @$filter["w_nr_meses"];
        $this->nr_meses->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->mes_ano_conv_coletiva;
        $searchFlds[] = &$this->sindicato;
        $searchFlds[] = &$this->cidade;
        $searchFlds[] = &$this->nr_meses;
        $searchFlds[] = &$this->obs;
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->idproposta->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->validade, $ctrl); // validade
            $this->updateSort($this->mes_ano_conv_coletiva, $ctrl); // mes_ano_conv_coletiva
            $this->updateSort($this->sindicato, $ctrl); // sindicato
            $this->updateSort($this->cidade, $ctrl); // cidade
            $this->updateSort($this->nr_meses, $ctrl); // nr_meses
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->mes_ano_conv_coletiva->setSort("");
                $this->sindicato->setSort("");
                $this->cidade->setSort("");
                $this->nr_meses->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_planilha_custo"
        $item = &$this->ListOptions->add("detail_planilha_custo");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'planilha_custo');
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;

        // "detail_mov_insumo_cliente"
        $item = &$this->ListOptions->add("detail_mov_insumo_cliente");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'mov_insumo_cliente');
        $item->OnLeft = true;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("planilha_custo");
        $pages->add("mov_insumo_cliente");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"proposta\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"proposta\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"proposta\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpropostalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpropostalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_planilha_custo"
        $opt = $this->ListOptions["detail_planilha_custo"];
        if ($Security->allowList(CurrentProjectID() . 'planilha_custo')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("planilha_custo", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("PlanilhaCustoList?" . Config("TABLE_SHOW_MASTER") . "=proposta&" . GetForeignKeyUrl("fk_idproposta", $this->idproposta->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PlanilhaCustoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=planilha_custo");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "planilha_custo";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=planilha_custo");
                ...
                $detailEditTblVar .= "planilha_custo";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=planilha_custo");
                ...
                $detailCopyTblVar .= "planilha_custo";
            }
            if ($links != "") {
        ...
        // "detail_mov_insumo_cliente"
        $opt = $this->ListOptions["detail_mov_insumo_cliente"];
        if ($Security->allowList(CurrentProjectID() . 'mov_insumo_cliente')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("mov_insumo_cliente", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("MovInsumoClienteList?" . Config("TABLE_SHOW_MASTER") . "=proposta&" . GetForeignKeyUrl("fk_idproposta", $this->idproposta->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("MovInsumoClienteGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=mov_insumo_cliente");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "mov_insumo_cliente";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=mov_insumo_cliente");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "mov_insumo_cliente";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'proposta')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=mov_insumo_cliente");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailCopyTblVar != "") {
                    $detailCopyTblVar .= ",";
                }
                $detailCopyTblVar .= "mov_insumo_cliente";
            }
            if ($links != "") {
                $body .= "<button type=\"button\" class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"proposta\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_planilha_custo");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=planilha_custo");
        $detailPage = Container("PlanilhaCustoGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'proposta') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "planilha_custo";
        }
        $item = &$option->add("detailadd_mov_insumo_cliente");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=mov_insumo_cliente");
        $detailPage = Container("MovInsumoClienteGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'proposta') && $Security->canAdd());
        if ($item->Visible) {
            ...
            $detailTableLink .= "mov_insumo_cliente";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "validade");
            $this->createColumnOption($option, "mes_ano_conv_coletiva");
            $this->createColumnOption($option, "sindicato");
            $this->createColumnOption($option, "cidade");
            $this->createColumnOption($option, "nr_meses");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpropostasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpropostasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpropostalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_proposta", "data-rowtype" => RowType::ADD]);
    ...
    protected function newRow()
    {
        ...
        $row['validade'] = $this->validade->DefaultValue;
        $row['mes_ano_conv_coletiva'] = $this->mes_ano_conv_coletiva->DefaultValue;
        $row['sindicato'] = $this->sindicato->DefaultValue;
        $row['cidade'] = $this->cidade->DefaultValue;
        $row['nr_meses'] = $this->nr_meses->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpropostasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
