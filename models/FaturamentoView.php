 ...
 * Page class
 */
class FaturamentoView extends Faturamento
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FaturamentoView";
    ...
    // CSS class/style
    public $CurrentPageName = "FaturamentoView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idfaturamento") ?? Route("idfaturamento")) !== null) {
            $this->RecKey["idfaturamento"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("idfaturamento") ?? Route("idfaturamento")) !== null) {
            ...
            $this->RecKey["idfaturamento"] = $this->idfaturamento->QueryStringValue;
        } elseif (Post("idfaturamento") !== null) {
            $this->idfaturamento->setFormValue(Post("idfaturamento"));
            $this->RecKey["idfaturamento"] = $this->idfaturamento->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->idfaturamento->setQueryStringValue($keyValue);
            $this->RecKey["idfaturamento"] = $this->idfaturamento->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "FaturamentoList"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "FaturamentoList"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_contato"
        $item = &$option->add("detail_contato");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("contato", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ContatoList?" . Config("TABLE_SHOW_MASTER") . "=faturamento&" . GetForeignKeyUrl("fk_idfaturamento", $this->idfaturamento->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("ContatoGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'faturamento')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=contato"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "contato";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'faturamento')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=contato"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "contato";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'faturamento')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=contato"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
