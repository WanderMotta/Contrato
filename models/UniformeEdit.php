 ...
 * Page class
 */
class UniformeEdit extends Uniforme
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "UniformeEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "UniformeEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("iduniforme") ?? Key(0) ?? Route(2)) !== null) {
                $this->iduniforme->setQueryStringValue($keyValue);
                $this->iduniforme->setOldValue($this->iduniforme->QueryStringValue);
            } elseif (Post("iduniforme") !== null) {
                $this->iduniforme->setFormValue(Post("iduniforme"));
                $this->iduniforme->setOldValue($this->iduniforme->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("iduniforme") ?? Route("iduniforme")) !== null) {
                    ...
                    $this->iduniforme->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->iduniforme->IsDetailKey) {
            $this->iduniforme->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->iduniforme->CurrentValue = $this->iduniforme->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->iduniforme->Visible && $this->iduniforme->Required) {
                if (!$this->iduniforme->IsDetailKey && EmptyValue($this->iduniforme->FormValue)) {
                    $this->iduniforme->addErrorMessage(str_replace("%s", $this->iduniforme->caption(), $this->iduniforme->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // uniforme
        $this->uniforme->setDbValueDef($rsnew, $this->uniforme->CurrentValue, $this->uniforme->ReadOnly);

        // qtde
        $this->qtde->setDbValueDef($rsnew, $this->qtde->CurrentValue, $this->qtde->ReadOnly);

        // periodo_troca
        $this->periodo_troca->setDbValueDef($rsnew, $this->periodo_troca->CurrentValue, $this->periodo_troca->ReadOnly);

        // vr_unitario
        $this->vr_unitario->setDbValueDef($rsnew, $this->vr_unitario->CurrentValue, $this->vr_unitario->ReadOnly);

        // vr_total
        $this->vr_total->setDbValueDef($rsnew, $this->vr_total->CurrentValue, $this->vr_total->ReadOnly);
        return $rsnew;
    }
