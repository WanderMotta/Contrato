 ...
 * Page class
 */
class MovimentoPlaCustoEdit extends MovimentoPlaCusto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MovimentoPlaCustoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "MovimentoPlaCustoEdit";
    ...
    {
        $this->idmovimento_pla_custo->setVisibility();
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idmovimento_pla_custo") ?? Key(0) ?? Route(2)) !== null) {
                $this->idmovimento_pla_custo->setQueryStringValue($keyValue);
                $this->idmovimento_pla_custo->setOldValue($this->idmovimento_pla_custo->QueryStringValue);
            } elseif (Post("idmovimento_pla_custo") !== null) {
                $this->idmovimento_pla_custo->setFormValue(Post("idmovimento_pla_custo"));
                $this->idmovimento_pla_custo->setOldValue($this->idmovimento_pla_custo->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("idmovimento_pla_custo") ?? Route("idmovimento_pla_custo")) !== null) {
                    ...
                    $this->idmovimento_pla_custo->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idmovimento_pla_custo->IsDetailKey) {
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->idmovimento_pla_custo->Visible && $this->idmovimento_pla_custo->Required) {
                if (!$this->idmovimento_pla_custo->IsDetailKey && EmptyValue($this->idmovimento_pla_custo->FormValue)) {
                    $this->idmovimento_pla_custo->addErrorMessage(str_replace("%s", $this->idmovimento_pla_custo->caption(), $this->idmovimento_pla_custo->RequiredErrorMessage));
                }
            }
