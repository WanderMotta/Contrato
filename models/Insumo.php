...
/**
 * Table class for insumo
 */
class Insumo extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // idinsumo
        $this->idinsumo = new DbField(
            $this, // Table
            'x_idinsumo', // Variable name
            'idinsumo', // Name
            '`idinsumo`', // Expression
            '`idinsumo`', // Basic search expression
            19, // Type
            3, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`idinsumo`', // Virtual expression
            false, // Is virtual
        ...
        $this->idinsumo->InputTextType = "text";
        $this->idinsumo->Raw = true;
        $this->idinsumo->IsAutoIncrement = true; // Autoincrement field
        $this->idinsumo->IsPrimaryKey = true; // Primary key field
        $this->idinsumo->Nullable = false; // NOT NULL field
        $this->idinsumo->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->idinsumo->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['idinsumo'] = &$this->idinsumo;
        ...
        $this->tipo_insumo_idtipo_insumo->Lookup = new Lookup($this->tipo_insumo_idtipo_insumo, 'tipo_insumo', false, 'idtipo_insumo', ["tipo_insumo","","",""], '', '', [], [], [], [], [], [], false, '`tipo_insumo` ASC', '', "`tipo_insumo`");
        ...
        $this->vr_unitario->addMethod("getDefault", fn() => 0.00);
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "insumo";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->idinsumo->setDbValue($conn->lastInsertId());
            $rs['idinsumo'] = $this->idinsumo->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['idinsumo']) && !EmptyValue($this->idinsumo->CurrentValue)) {
                $rs['idinsumo'] = $this->idinsumo->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('idinsumo', $rs)) {
                AddFilter($where, QuotedName('idinsumo', $this->Dbid) . '=' . QuotedValue($rs['idinsumo'], $this->idinsumo->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->idinsumo->DbValue = $row['idinsumo'];
    ...
    {
        $keys = [];
        $val = $current ? $this->idinsumo->CurrentValue : $this->idinsumo->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idinsumo->CurrentValue = $keys[0];
            } else {
                $this->idinsumo->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('idinsumo', $row) ? $row['idinsumo'] : null;
        } else {
            $val = !EmptyValue($this->idinsumo->OldValue) && !$current ? $this->idinsumo->OldValue : $this->idinsumo->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@idinsumo@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InsumoList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "InsumoView" => $Language->phrase("View"),
            "InsumoEdit" => $Language->phrase("Edit"),
            "InsumoAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "InsumoView",
            Config("API_ADD_ACTION") => "InsumoAdd",
            Config("API_EDIT_ACTION") => "InsumoEdit",
            Config("API_DELETE_ACTION") => "InsumoDelete",
            Config("API_LIST_ACTION") => "InsumoList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InsumoView", $parm);
        } else {
            $url = $this->keyUrl("InsumoView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InsumoEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InsumoList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InsumoAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InsumoList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("InsumoDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"idinsumo\":" . VarToJson($this->idinsumo->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->idinsumo->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->idinsumo->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("idinsumo") ?? Route("idinsumo")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idinsumo->CurrentValue = $key;
            } else {
                $this->idinsumo->OldValue = $key;
            }
    ...
    public function exportDocument($doc, $result, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->idinsumo);
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->idinsumo, '');
                $doc->exportAggregate($this->insumo, 'COUNT');
