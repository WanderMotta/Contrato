 ...
 * Page class
 */
class ViewRelCargoCustoIndvList extends ViewRelCargoCustoIndv
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewRelCargoCustoIndvList";
    ...
    public $FormName = "fview_rel_cargo_custo_indvlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewRelCargoCustoIndvList";
    ...
    public function setVisibility()
    {
        ...
        $this->vr_contrato->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_rel_cargo_custo_indv';
        $this->TableName = 'view_rel_cargo_custo_indv';
        ...
        // Table object (view_rel_cargo_custo_indv)
        if (!isset($GLOBALS["view_rel_cargo_custo_indv"]) || $GLOBALS["view_rel_cargo_custo_indv"]::class == PROJECT_NAMESPACE . "view_rel_cargo_custo_indv") {
            $GLOBALS["view_rel_cargo_custo_indv"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewRelCargoCustoIndvAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewRelCargoCustoIndvDelete";
        $this->MultiUpdateUrl = "ViewRelCargoCustoIndvUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_rel_cargo_custo_indv');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewRelCargoCustoIndvView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_rel_cargo_custo_indvgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->vr_contrato->AdvancedSearch->toJson(), ","); // Field vr_contrato
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_rel_cargo_custo_indvsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field vr_contrato
        $this->vr_contrato->AdvancedSearch->SearchValue = @$filter["x_vr_contrato"];
        $this->vr_contrato->AdvancedSearch->SearchOperator = @$filter["z_vr_contrato"];
        $this->vr_contrato->AdvancedSearch->SearchCondition = @$filter["v_vr_contrato"];
        $this->vr_contrato->AdvancedSearch->SearchValue2 = @$filter["y_vr_contrato"];
        $this->vr_contrato->AdvancedSearch->SearchOperator2 = @$filter["w_vr_contrato"];
        $this->vr_contrato->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->vr_contrato, $ctrl); // vr_contrato
            $this->updateSort($this->cargo, $ctrl); // cargo
            $this->updateSort($this->idmodulo, $ctrl); // idmodulo
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_rel_cargo_custo_indvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_rel_cargo_custo_indvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "vr_contrato");
            $this->createColumnOption($option, "cargo");
            $this->createColumnOption($option, "idmodulo");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_rel_cargo_custo_indvsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_rel_cargo_custo_indvsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_rel_cargo_custo_indvlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_rel_cargo_custo_indv", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_rel_cargo_custo_indv",
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->vr_contrato->setDbValue($row['vr_contrato']);
    ...
    protected function newRow()
    {
        ...
        $row['vr_contrato'] = $this->vr_contrato->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // vr_contrato
            $this->vr_contrato->ViewValue = $this->vr_contrato->CurrentValue;
            $this->vr_contrato->ViewValue = FormatNumber($this->vr_contrato->ViewValue, $this->vr_contrato->formatPattern());
            ...
            $this->idmodulo->ViewValue = FormatNumber($this->idmodulo->ViewValue, $this->idmodulo->formatPattern());
            ...
            // vr_contrato
            $this->vr_contrato->HrefValue = "";
            $this->vr_contrato->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fview_rel_cargo_custo_indvlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fview_rel_cargo_custo_indvlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fview_rel_cargo_custo_indvlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_rel_cargo_custo_indvlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_rel_cargo_custo_indvsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
