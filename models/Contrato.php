...
class Contrato extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->idcontrato->IsAutoIncrement = true; // Autoincrement field
        $this->idcontrato->IsPrimaryKey = true; // Primary key field
        $this->idcontrato->IsForeignKey = true; // Foreign key field
        ...
        $this->cliente_idcliente->addMethod("getSelectFilter", fn() => "`ativo`='Sim'");
        ...
        $this->ativo->Lookup = new Lookup($this->ativo, 'contrato', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        // valor_antes
        $this->valor_antes = new DbField(
            $this, // Table
            'x_valor_antes', // Variable name
            'valor_antes', // Name
            '`valor_antes`', // Expression
            '`valor_antes`', // Basic search expression
            131, // Type
            12, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`valor_antes`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->valor_antes->addMethod("getDefault", fn() => 0.00);
        $this->valor_antes->InputTextType = "text";
        $this->valor_antes->Raw = true;
        $this->valor_antes->Nullable = false; // NOT NULL field
        $this->valor_antes->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->valor_antes->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['valor_antes'] = &$this->valor_antes;
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "planilha_custo_contrato") {
            $detailUrl = Container("planilha_custo_contrato")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_idcontrato", $this->idcontrato->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "mov_insumo_contrato") {
            $detailUrl = Container("mov_insumo_contrato")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_idcontrato", $this->idcontrato->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "ContratoList";
        }
        return $detailUrl;
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->idcontrato->setDbValue($conn->lastInsertId());
            $rs['idcontrato'] = $this->idcontrato->DbValue;
        }
        return $result;
    }
    ...
    {
        // Cascade Update detail table 'planilha_custo_contrato'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['idcontrato']) && $rsold['idcontrato'] != $rs['idcontrato'])) { // Update detail field 'contrato_idcontrato'
            $cascadeUpdate = true;
            $rscascade['contrato_idcontrato'] = $rs['idcontrato'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("planilha_custo_contrato")->loadRs("`contrato_idcontrato` = " . QuotedValue($rsold['idcontrato'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'idplanilha_custo_contrato';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("planilha_custo_contrato")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("planilha_custo_contrato")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("planilha_custo_contrato")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'mov_insumo_contrato'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['idcontrato']) && $rsold['idcontrato'] != $rs['idcontrato'])) { // Update detail field 'contrato_idcontrato'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("mov_insumo_contrato")->loadRs("`contrato_idcontrato` = " . QuotedValue($rsold['idcontrato'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'idmov_insumo_contrato';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("mov_insumo_contrato")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("mov_insumo_contrato")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("mov_insumo_contrato")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
        ...
        if ($success) {
            if (!isset($rs['idcontrato']) && !EmptyValue($this->idcontrato->CurrentValue)) {
                $rs['idcontrato'] = $this->idcontrato->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('idcontrato', $rs)) {
                AddFilter($where, QuotedName('idcontrato', $this->Dbid) . '=' . QuotedValue($rs['idcontrato'], $this->idcontrato->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'planilha_custo_contrato'
        $dtlrows = Container("planilha_custo_contrato")->loadRs("`contrato_idcontrato` = " . QuotedValue($rs['idcontrato'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("planilha_custo_contrato")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("planilha_custo_contrato")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("planilha_custo_contrato")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'mov_insumo_contrato'
        $dtlrows = Container("mov_insumo_contrato")->loadRs("`contrato_idcontrato` = " . QuotedValue($rs['idcontrato'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("mov_insumo_contrato")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("mov_insumo_contrato")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("mov_insumo_contrato")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
            try {
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->valor_antes->DbValue = $row['valor_antes'];
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->idcontrato->CurrentValue : $this->idcontrato->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idcontrato->CurrentValue = $keys[0];
            } else {
                $this->idcontrato->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('idcontrato', $row) ? $row['idcontrato'] : null;
        } else {
            $val = !EmptyValue($this->idcontrato->OldValue) && !$current ? $this->idcontrato->OldValue : $this->idcontrato->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@idcontrato@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ContratoView" => $Language->phrase("View"),
            "ContratoEdit" => $Language->phrase("Edit"),
            "ContratoAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ContratoView",
            Config("API_ADD_ACTION") => "ContratoAdd",
            Config("API_EDIT_ACTION") => "ContratoEdit",
            Config("API_DELETE_ACTION") => "ContratoDelete",
            Config("API_LIST_ACTION") => "ContratoList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContratoView", $parm);
        } else {
            $url = $this->keyUrl("ContratoView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContratoEdit", $parm);
        } else {
            $url = $this->keyUrl("ContratoEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContratoList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContratoAdd", $parm);
        } else {
            $url = $this->keyUrl("ContratoAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContratoList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ContratoDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"idcontrato\":" . VarToJson($this->idcontrato->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->idcontrato->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->idcontrato->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("idcontrato") ?? Route("idcontrato")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idcontrato->CurrentValue = $key;
            } else {
                $this->idcontrato->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // valor_antes
        $this->valor_antes->HrefValue = "";
        $this->valor_antes->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // valor_antes
        $this->valor_antes->setupEditAttributes();
        $this->valor_antes->EditValue = $this->valor_antes->CurrentValue;
        $this->valor_antes->EditValue = FormatNumber($this->valor_antes->EditValue, $this->valor_antes->formatPattern());
        $this->valor_antes->CssClass = "fw-bold";
        $this->valor_antes->CellCssStyle .= "text-align: right;";
    ...
    public function exportDocument($doc, $result, $startRec = 1, $stopRec = 1, $exportPageType = "")
    {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->valor_antes);
                } else {
                    $doc->exportCaption($this->idcontrato);
                    $doc->exportCaption($this->cliente_idcliente);
                    $doc->exportCaption($this->valor);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->valor_antes);
                    } else {
                        $doc->exportField($this->idcontrato);
                        $doc->exportField($this->cliente_idcliente);
                        $doc->exportField($this->valor);
        ...
        if ($doc->Horizontal) {
            ...
            if (!$doc->ExportCustom) {
                ...
                $doc->exportAggregate($this->idcontrato, '');
                ...
                $doc->exportAggregate($this->valor_antes, '');
                $doc->endExportRow();
            }
        }
        if (!$doc->ExportCustom) {
