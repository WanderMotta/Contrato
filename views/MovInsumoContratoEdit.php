...
?>
<main class="edit">
<form name="fmov_insumo_contratoedit" id="fmov_insumo_contratoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { mov_insumo_contrato: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fmov_insumo_contratoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmov_insumo_contratoedit")
        .setPageId("edit")
...
<?php if ($Page->idmov_insumo_contrato->Visible) { // idmov_insumo_contrato ?>
    <div id="r_idmov_insumo_contrato"<?= $Page->idmov_insumo_contrato->rowAttributes() ?>>
        <label id="elh_mov_insumo_contrato_idmov_insumo_contrato" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idmov_insumo_contrato->caption() ?><?= $Page->idmov_insumo_contrato->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idmov_insumo_contrato->cellAttributes() ?>>
<span id="el_mov_insumo_contrato_idmov_insumo_contrato">
...
<input type="hidden" data-table="mov_insumo_contrato" data-field="x_idmov_insumo_contrato" data-hidden="1" name="x_idmov_insumo_contrato" id="x_idmov_insumo_contrato" value="<?= HtmlEncode($Page->idmov_insumo_contrato->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_mov_insumo_contrato_insumo_idinsumo">
    <select
        ...
        data-select2-id="fmov_insumo_contratoedit_x_insumo_idinsumo"
        data-table="mov_insumo_contrato"
...
<script>
loadjs.ready("fmov_insumo_contratoedit", function() {
    var options = { name: "x_insumo_idinsumo", selectId: "fmov_insumo_contratoedit_x_insumo_idinsumo" };
    if (fmov_insumo_contratoedit.lists.insumo_idinsumo?.lookupOptions.length) {
        options.data = { id: "x_insumo_idinsumo", form: "fmov_insumo_contratoedit" };
    } else {
        options.ajax = { id: "x_insumo_idinsumo", form: "fmov_insumo_contratoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->vr_unit->Visible) { // vr_unit ?>
    <div id="r_vr_unit"<?= $Page->vr_unit->rowAttributes() ?>>
        <label id="elh_mov_insumo_contrato_vr_unit" for="x_vr_unit" class="<?= $Page->LeftColumnClass ?>"><?= $Page->vr_unit->caption() ?><?= $Page->vr_unit->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->vr_unit->cellAttributes() ?>>
<span id="el_mov_insumo_contrato_vr_unit">
...
<input type="hidden" data-table="mov_insumo_contrato" data-field="x_vr_unit" data-hidden="1" name="x_vr_unit" id="x_vr_unit" value="<?= HtmlEncode($Page->vr_unit->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmov_insumo_contratoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmov_insumo_contratoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
