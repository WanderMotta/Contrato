...
?>
<main class="edit">
<form name="finsumoedit" id="finsumoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idinsumo", [fields.idinsumo.visible && fields.idinsumo.required ? ew.Validators.required(fields.idinsumo.caption) : null], fields.idinsumo.isInvalid],
...
<?php if ($Page->idinsumo->Visible) { // idinsumo ?>
    <div id="r_idinsumo"<?= $Page->idinsumo->rowAttributes() ?>>
        <label id="elh_insumo_idinsumo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idinsumo->caption() ?><?= $Page->idinsumo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idinsumo->cellAttributes() ?>>
<span id="el_insumo_idinsumo">
<span<?= $Page->idinsumo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idinsumo->getDisplayValue($Page->idinsumo->EditValue))) ?>"></span>
<input type="hidden" data-table="insumo" data-field="x_idinsumo" data-hidden="1" name="x_idinsumo" id="x_idinsumo" value="<?= HtmlEncode($Page->idinsumo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="finsumoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="finsumoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
