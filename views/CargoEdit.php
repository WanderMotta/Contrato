...
?>
<main class="edit">
<form name="fcargoedit" id="fcargoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_CargoEdit"><!-- multi-page tabs -->
...
<?php if ($Page->idcargo->Visible) { // idcargo ?>
    <div id="r_idcargo"<?= $Page->idcargo->rowAttributes() ?>>
        <label id="elh_cargo_idcargo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idcargo->caption() ?><?= $Page->idcargo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idcargo->cellAttributes() ?>>
<span id="el_cargo_idcargo">
...
<input type="hidden" data-table="cargo" data-field="x_idcargo" data-hidden="1" data-page="1" name="x_idcargo" id="x_idcargo" value="<?= HtmlEncode($Page->idcargo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_cargo_funcao_idfuncao">
    <select
        ...
        data-select2-id="fcargoedit_x_funcao_idfuncao"
        data-table="cargo"
...
loadjs.ready("fcargoedit", function() {
    var options = { name: "x_funcao_idfuncao", selectId: "fcargoedit_x_funcao_idfuncao" };
    if (fcargoedit.lists.funcao_idfuncao?.lookupOptions.length) {
        options.data = { id: "x_funcao_idfuncao", form: "fcargoedit" };
    } else {
        options.ajax = { id: "x_funcao_idfuncao", form: "fcargoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_cargo_tipo_uniforme_idtipo_uniforme">
    <select
        ...
        data-select2-id="fcargoedit_x_tipo_uniforme_idtipo_uniforme"
        data-table="cargo"
...
<script>
loadjs.ready("fcargoedit", function() {
    var options = { name: "x_tipo_uniforme_idtipo_uniforme", selectId: "fcargoedit_x_tipo_uniforme_idtipo_uniforme" };
    if (fcargoedit.lists.tipo_uniforme_idtipo_uniforme?.lookupOptions.length) {
        options.data = { id: "x_tipo_uniforme_idtipo_uniforme", form: "fcargoedit" };
    } else {
        options.ajax = { id: "x_tipo_uniforme_idtipo_uniforme", form: "fcargoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->modulo_idmodulo->Visible) { // modulo_idmodulo ?>
    <div id="r_modulo_idmodulo"<?= $Page->modulo_idmodulo->rowAttributes() ?>>
        <label id="elh_cargo_modulo_idmodulo" for="x_modulo_idmodulo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->modulo_idmodulo->caption() ?><?= $Page->modulo_idmodulo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->modulo_idmodulo->cellAttributes() ?>>
<span id="el_cargo_modulo_idmodulo">
<span<?= $Page->modulo_idmodulo->viewAttributes() ?>>
<span class="form-control-plaintext"><?= $Page->modulo_idmodulo->getDisplayValue($Page->modulo_idmodulo->EditValue) ?></span></span>
<input type="hidden" data-table="cargo" data-field="x_modulo_idmodulo" data-hidden="1" data-page="1" name="x_modulo_idmodulo" id="x_modulo_idmodulo" value="<?= HtmlEncode($Page->modulo_idmodulo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcargoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcargoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
