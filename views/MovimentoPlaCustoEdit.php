...
?>
<main class="edit">
<form name="fmovimento_pla_custoedit" id="fmovimento_pla_custoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { movimento_pla_custo: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fmovimento_pla_custoedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmovimento_pla_custoedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["idmovimento_pla_custo", [fields.idmovimento_pla_custo.visible && fields.idmovimento_pla_custo.required ? ew.Validators.required(fields.idmovimento_pla_custo.caption) : null], fields.idmovimento_pla_custo.isInvalid],
...
<?php if ($Page->idmovimento_pla_custo->Visible) { // idmovimento_pla_custo ?>
    <div id="r_idmovimento_pla_custo"<?= $Page->idmovimento_pla_custo->rowAttributes() ?>>
        <label id="elh_movimento_pla_custo_idmovimento_pla_custo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idmovimento_pla_custo->caption() ?><?= $Page->idmovimento_pla_custo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idmovimento_pla_custo->cellAttributes() ?>>
<span id="el_movimento_pla_custo_idmovimento_pla_custo">
<span<?= $Page->idmovimento_pla_custo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idmovimento_pla_custo->getDisplayValue($Page->idmovimento_pla_custo->EditValue))) ?>"></span>
<input type="hidden" data-table="movimento_pla_custo" data-field="x_idmovimento_pla_custo" data-hidden="1" name="x_idmovimento_pla_custo" id="x_idmovimento_pla_custo" value="<?= HtmlEncode($Page->idmovimento_pla_custo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_movimento_pla_custo_modulo_idmodulo">
    <select
        ...
        data-select2-id="fmovimento_pla_custoedit_x_modulo_idmodulo"
...
<script>
loadjs.ready("fmovimento_pla_custoedit", function() {
    var options = { name: "x_modulo_idmodulo", selectId: "fmovimento_pla_custoedit_x_modulo_idmodulo" };
    if (fmovimento_pla_custoedit.lists.modulo_idmodulo?.lookupOptions.length) {
        options.data = { id: "x_modulo_idmodulo", form: "fmovimento_pla_custoedit" };
    } else {
        options.ajax = { id: "x_modulo_idmodulo", form: "fmovimento_pla_custoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_movimento_pla_custo_itens_modulo_iditens_modulo">
    <select
        ...
        data-select2-id="fmovimento_pla_custoedit_x_itens_modulo_iditens_modulo"
        data-table="movimento_pla_custo"
...
<script>
loadjs.ready("fmovimento_pla_custoedit", function() {
    var options = { name: "x_itens_modulo_iditens_modulo", selectId: "fmovimento_pla_custoedit_x_itens_modulo_iditens_modulo" };
    if (fmovimento_pla_custoedit.lists.itens_modulo_iditens_modulo?.lookupOptions.length) {
        options.data = { id: "x_itens_modulo_iditens_modulo", form: "fmovimento_pla_custoedit" };
    } else {
        options.ajax = { id: "x_itens_modulo_iditens_modulo", form: "fmovimento_pla_custoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmovimento_pla_custoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmovimento_pla_custoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
