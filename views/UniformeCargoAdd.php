...
$Page->showMessage();
?>
<form name="funiforme_cargoadd" id="funiforme_cargoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_uniforme_cargo_uniforme_iduniforme">
    <select
        ...
        data-select2-id="funiforme_cargoadd_x_uniforme_iduniforme"
...
<script>
loadjs.ready("funiforme_cargoadd", function() {
    var options = { name: "x_uniforme_iduniforme", selectId: "funiforme_cargoadd_x_uniforme_iduniforme" };
    if (funiforme_cargoadd.lists.uniforme_iduniforme?.lookupOptions.length) {
        options.data = { id: "x_uniforme_iduniforme", form: "funiforme_cargoadd" };
    } else {
        options.ajax = { id: "x_uniforme_iduniforme", form: "funiforme_cargoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="funiforme_cargoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="funiforme_cargoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
