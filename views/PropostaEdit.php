...
?>
<main class="edit">
<form name="fpropostaedit" id="fpropostaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idproposta", [fields.idproposta.visible && fields.idproposta.required ? ew.Validators.required(fields.idproposta.caption) : null], fields.idproposta.isInvalid],
...
<?php if ($Page->idproposta->Visible) { // idproposta ?>
    <div id="r_idproposta"<?= $Page->idproposta->rowAttributes() ?>>
        <label id="elh_proposta_idproposta" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idproposta->caption() ?><?= $Page->idproposta->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idproposta->cellAttributes() ?>>
<span id="el_proposta_idproposta">
<span<?= $Page->idproposta->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idproposta->getDisplayValue($Page->idproposta->EditValue))) ?>"></span>
<input type="hidden" data-table="proposta" data-field="x_idproposta" data-hidden="1" name="x_idproposta" id="x_idproposta" value="<?= HtmlEncode($Page->idproposta->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_proposta_cliente_idcliente">
    <select
        ...
        data-select2-id="fpropostaedit_x_cliente_idcliente"
        data-table="proposta"
...
<script>
loadjs.ready("fpropostaedit", function() {
    var options = { name: "x_cliente_idcliente", selectId: "fpropostaedit_x_cliente_idcliente" };
    if (fpropostaedit.lists.cliente_idcliente?.lookupOptions.length) {
        options.data = { id: "x_cliente_idcliente", form: "fpropostaedit" };
    } else {
        options.ajax = { id: "x_cliente_idcliente", form: "fpropostaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fpropostaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpropostaedit", "x_validade", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("planilha_custo", explode(",", $Page->getCurrentDetailTable())) && $planilha_custo->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("mov_insumo_cliente", explode(",", $Page->getCurrentDetailTable())) && $mov_insumo_cliente->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpropostaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpropostaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
