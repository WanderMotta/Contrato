...
?>
<main class="edit">
<form name="fperiodoedit" id="fperiodoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["idperiodo", [fields.idperiodo.visible && fields.idperiodo.required ? ew.Validators.required(fields.idperiodo.caption) : null], fields.idperiodo.isInvalid],
...
<?php if ($Page->idperiodo->Visible) { // idperiodo ?>
    <div id="r_idperiodo"<?= $Page->idperiodo->rowAttributes() ?>>
        <label id="elh_periodo_idperiodo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idperiodo->caption() ?><?= $Page->idperiodo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->idperiodo->cellAttributes() ?>>
<span id="el_periodo_idperiodo">
<span<?= $Page->idperiodo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idperiodo->getDisplayValue($Page->idperiodo->EditValue))) ?>"></span>
<input type="hidden" data-table="periodo" data-field="x_idperiodo" data-hidden="1" name="x_idperiodo" id="x_idperiodo" value="<?= HtmlEncode($Page->idperiodo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fperiodoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fperiodoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
