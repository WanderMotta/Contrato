...
?>
<main class="edit">
<form name="fitens_moduloedit" id="fitens_moduloedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { itens_modulo: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fitens_moduloedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fitens_moduloedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["iditens_modulo", [fields.iditens_modulo.visible && fields.iditens_modulo.required ? ew.Validators.required(fields.iditens_modulo.caption) : null], fields.iditens_modulo.isInvalid],
...
<?php if ($Page->iditens_modulo->Visible) { // iditens_modulo ?>
    <div id="r_iditens_modulo"<?= $Page->iditens_modulo->rowAttributes() ?>>
        <label id="elh_itens_modulo_iditens_modulo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->iditens_modulo->caption() ?><?= $Page->iditens_modulo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->iditens_modulo->cellAttributes() ?>>
<span id="el_itens_modulo_iditens_modulo">
<span<?= $Page->iditens_modulo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->iditens_modulo->getDisplayValue($Page->iditens_modulo->EditValue))) ?>"></span>
<input type="hidden" data-table="itens_modulo" data-field="x_iditens_modulo" data-hidden="1" name="x_iditens_modulo" id="x_iditens_modulo" value="<?= HtmlEncode($Page->iditens_modulo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_itens_modulo_modulo_idmodulo">
    <select
        ...
        data-select2-id="fitens_moduloedit_x_modulo_idmodulo"
        <?php } ?>
...
<?php if (!$Page->modulo_idmodulo->IsNativeSelect) { ?>
<script>
loadjs.ready("fitens_moduloedit", function() {
    var options = { name: "x_modulo_idmodulo", selectId: "fitens_moduloedit_x_modulo_idmodulo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fitens_moduloedit.lists.modulo_idmodulo?.lookupOptions.length) {
        options.data = { id: "x_modulo_idmodulo", form: "fitens_moduloedit" };
    } else {
        options.ajax = { id: "x_modulo_idmodulo", form: "fitens_moduloedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->ativo->Visible) { // ativo ?>
    <div id="r_ativo"<?= $Page->ativo->rowAttributes() ?>>
        <label id="elh_itens_modulo_ativo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ativo->caption() ?><?= $Page->ativo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->ativo->cellAttributes() ?>>
<span id="el_itens_modulo_ativo">
<template id="tp_x_ativo">
    <div class="form-check">
        <input type="radio" class="form-check-input" data-table="itens_modulo" data-field="x_ativo" name="x_ativo" id="x_ativo"<?= $Page->ativo->editAttributes() ?>>
        <label class="form-check-label"></label>
    </div>
</template>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fitens_moduloedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fitens_moduloedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
