...
?>
<main class="edit">
<form name="fdissidio_anualedit" id="fdissidio_anualedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { dissidio_anual: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdissidio_anualedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdissidio_anualedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["iddissidio_anual", [fields.iddissidio_anual.visible && fields.iddissidio_anual.required ? ew.Validators.required(fields.iddissidio_anual.caption) : null], fields.iddissidio_anual.isInvalid],
...
<?php if ($Page->iddissidio_anual->Visible) { // iddissidio_anual ?>
    <div id="r_iddissidio_anual"<?= $Page->iddissidio_anual->rowAttributes() ?>>
        <label id="elh_dissidio_anual_iddissidio_anual" class="<?= $Page->LeftColumnClass ?>"><?= $Page->iddissidio_anual->caption() ?><?= $Page->iddissidio_anual->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->iddissidio_anual->cellAttributes() ?>>
<span id="el_dissidio_anual_iddissidio_anual">
<span<?= $Page->iddissidio_anual->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->iddissidio_anual->getDisplayValue($Page->iddissidio_anual->EditValue))) ?>"></span>
<input type="hidden" data-table="dissidio_anual" data-field="x_iddissidio_anual" data-hidden="1" name="x_iddissidio_anual" id="x_iddissidio_anual" value="<?= HtmlEncode($Page->iddissidio_anual->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fdissidio_anualedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdissidio_anualedit", "x_data", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("dissidio", explode(",", $Page->getCurrentDetailTable())) && $dissidio->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("beneficios", explode(",", $Page->getCurrentDetailTable())) && $beneficios->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdissidio_anualedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdissidio_anualedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
